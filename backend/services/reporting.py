...
class ReportGenerator:
    @staticmethod
    def generate_pdf_report(report_type, machine_id=None, start_date=None, end_date=None, generated_by=None):
        ...
        if report_type == 'work_orders':
            ReportGenerator._add_work_orders_content(elements, styles, machine_id, start_date, end_date)
        elif report_type == 'failures':
            ReportGenerator._add_failures_content(elements, styles, machine_id, start_date, end_date)
        elif report_type == 'statistics':
            ReportGenerator._add_statistics_content(elements, styles, machine_id, start_date, end_date)
        elif report_type == 'maintenance':
            ReportGenerator._add_maintenance_content(elements, styles, machine_id, start_date, end_date)
    ...
    def _add_work_orders_content(elements, styles, machine_id, start_date, end_date):
        ...
        # Add detailed work orders if not too many
    ...
    @staticmethod
    def _add_failures_content(elements, styles, machine_id, start_date, end_date):
        """Add failures report content"""
        elements.append(Paragraph("Failures Summary", styles['Heading2']))
        elements.append(Spacer(1, 12))
        
        # Get failure statistics (similar to failure rates)
        failure_rates = MaintenanceStatistics.get_failure_rates(machine_id, start_date, end_date)
        
        # Create summary table
        summary_data = [
            ['Machine', 'Total Failures', 'Operation Hours', 'Failure Rate (per 1000h)']
        ]
        ...
        # Add totals row
        avg_failure_rate = (total_failures / total_hours * 1000) if total_hours > 0 else 0
        summary_data.append([
            'TOTAL',
            total_failures,
            total_hours,
            round(avg_failure_rate, 2)
        ])
        ...
        query = db.session.query(
            ...
            User.username.label('reported_by')
        ).join(
            MaintenanceLog, Failure.maintenance_log_id == MaintenanceLog.id
        ).join(
        ...
        failures = query.order_by(MaintenanceLog.timestamp.desc()).all()
        
        if failures:
            elements.append(Paragraph("Recent Failures", styles['Heading2']))
            elements.append(Spacer(1, 12))
            
            # Add detailed failure information
            for failure_data in failures[:20]:  # Limit to 20 most recent failures
                failure = failure_data[0]
                maintenance_log = failure_data[1]
                machine_name = failure_data.machine_name
                reported_by = failure_data.reported_by
                
                # Failure details paragraph
                failure_title = f"Failure #{failure.id}: {machine_name} ({failure.severity.upper()})"
                elements.append(Paragraph(failure_title, styles['Heading3']))
                
                details = [
                    f"Reported by: {reported_by}",
                    f"Date: {maintenance_log.timestamp.strftime('%Y-%m-%d %H:%M')}",
                    f"Description: {failure.description}"
                ]
                
                if failure.resolution:
                    details.append(f"Resolution: {failure.resolution}")
